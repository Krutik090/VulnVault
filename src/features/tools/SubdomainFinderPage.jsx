/**
 * ======================================================================
 * FILE: src/features/tools/SubdomainFinderPage.jsx (FULLY CORRECTED)
 * PURPOSE: Subdomain Finder with role-based access control
 * SOC 2: Access control, audit logging, secure scanning
 * ======================================================================
 */
...
const SubdomainFinderPage = () => {
        ...
        try {
            ...
            for (const protocol of protocols) {
                try {
                    ...
                    await fetch(`${protocol}://${subdomain}`, {
                        method: 'HEAD',
                        mode: 'no-cors',
    ...
    const checkAllDomainsStatus = useCallback(async () => {
        ...
        // ✅ FIXED: Use optional chaining for safe user.email access
        console.log(
            `🔍 Starting status check for ${results.length} subdomains by ${user?.email}`
        );
        ...
        try {
            ...
            const aliveCount = results.filter((r) => r.status === 'alive')
                .length;
            console.log(
    ...
    }, [results, user?.email]); // ✅ FIXED: Use optional chaining in dependency
    ...
    const handleSubmit = async (e) => {
        ...
        // ✅ FIXED: Use optional chaining for safe user.email access
        console.log(
            `🔍 Starting subdomain scan for: ${cleanDomain} by ${user?.email}`
        );
        ...
        try {
            ...
            console.log(
                `✅ Found ${subdomains.length} subdomains for ${cleanDomain}`
            );
            ...
            if (!subdomains || subdomains.length === 0) {
                toast.success(
                    'Scan completed. No subdomains found for this domain.'
                );
            } else {
                toast.success(
            ...
            toast.error(
                error.message ||
                    'Failed to find subdomains. Please try again.'
            );
    ...
    const handleReset = () => {
        // ✅ FIXED: Use optional chaining for safe user.email access
        console.log(`🔄 Resetting subdomain finder for ${user?.email}`);
        setDomain('');
        setResults([]);
        setHasSearched(false);
        setIsCheckingStatus(false);
                ...
                {isLoading && (
                    <div className="bg-card border border-border rounded-lg p-8 text-center">
                        ...
                        <p className="text-sm text-muted-foreground">
                            This may take a few moments. Please wait while we
                            discover subdomains.
                        </p>
                    </div>
                )}
                    ...
                    <div className="bg-card border border-border rounded-lg p-8 text-center">
                        <div className="text-muted-foreground mb-4">
                            ...
                            <p className="text-sm">
                                ...
                                could mean the domain has no public subdomains
                                or they are not indexed in our databases.
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div className="bg-muted/30 border border-border rounded-lg p-6">
                    ...
                    <div className="grid md:grid-cols-3 gap-6 text-sm text-muted-foreground">
                        <div>
                            ...
                            <p>
                                Subdomain enumeration is the process of discovering
                                subdomains for a given domain. It's a crucial step in
                                reconnaissance during penetration testing to map the
                                attack surface.
                            </p>
                        </div>
                        <div>
                            ...
                            <p>
                                The status check feature verifies if discovered
                                subdomains are alive by sending HTTP/HTTPS requests.
                                This helps identify active services and potential
